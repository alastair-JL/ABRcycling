...
lockedParams(2)=sum(m)/lockedParams(1);
m=[m(1:3),0,m(4)]'/sum(m); %InfluxRate S,A,B,AB, X

chi=[0,0,1,0];

 ABRsim= @(t,y) ABRsimulationCycle(chi,y,lockedParams,t,T,m);
...
y0(5)=sigma-sum(y0);
 ...
 [t,yQQ]=ode45(ABRsim, linspace(0,T*20,10000), y0, odeset('NonNegative',1))
 figure(44);
