...
durgCorrection=0; %Rate at which incorrect drug use is replaced by correct drug use.
...
if(chi>-1)
    ...
    filter= [1;0;1;1;0;1;1]; %Combination therapy: Everything always has an effective therapy.
end
...
% 
% figure()
% plot(ChiList(1:(end-1)),DoesThisIntegrateToOne(1:(end-1)))
% 
% figure(11)
% plot(ChiList(1:(end-1)),RhoObserved(1:(end-1)));
% hold on
% plot(ChiList(1:(end-1)),RhoPredicted(1:(end-1)),':');
% xlabel('\chi_A')
% ylabel('\rho')
% 
% figure(5)
% subplot(2,1,1)
% plot(ChiList(1:(end-1)),MutantArrivalRates)
% xlabel('\chi_A')
% ylabel('M')
% legend({'import','base','select','HGT'});
% 
% subplot(2,1,2)
% plot(ChiList(1:(end-1)),X_bar)
% hold on
% plot(ChiList(1:(end-1)),0*X_bar+g/beta(4),'k:');
% xlabel('\chi_A')
% ylabel('\bar X')
...
figure(7)
subplot(3,2,1)
plot(ChiList(1:(end-1)),X365(1:(end-1),:),'lineWidth',1 )
set(gca, 'ColorOrder', StandardColours,'NextPlot', 'replacechildren');
hold on
plot([0,0.25,0.5,0.75,1],[1,1,1,1,1].*X365(end,:)','+','lineWidth',1 )
...
subplot(3,2,2)
plot(ChiList(1:(end-1)),X365_lowMutant(1:(end-1),:),'lineWidth',1 )
set(gca, 'ColorOrder', StandardColours,'NextPlot', 'replacechildren');
hold on
plot([0,0.25,0.5,0.75,1],[1,1,1,1,1].*X365_lowMutant(end,:)','+','lineWidth',1 )
...
subplot(3,2,3)
plot(ChiList(1:(end-1)),Expected_T_half(1:(end-1),:),'lineWidth',1 )
set(gca, 'ColorOrder', StandardColours,'NextPlot', 'replacechildren');
hold on
plot([0,0.25,0.5,0.75,1],[1,1,1,1,1].*Expected_T_half(end,:)','+','lineWidth',1 )

xlabel('\chi_A')
ylabel('T_{1/2}')

subplot(3,2,4)
plot(ChiList(1:(end-1)),X_T(1:(end-1),:),'lineWidth',1 )
set(gca, 'ColorOrder', StandardColours,'NextPlot', 'replacechildren');
hold on
plot([0,0.25,0.5,0.75,1],[1,1,1,1,1].*X_T(end,:)','+','lineWidth',1 )
xlabel('\chi_A')
ylabel('X_{T}')

subplot(3,2,5)
plot(ChiList(1:(end-1)),X_Tstar(1:(end-1),:),'lineWidth',1 )
set(gca, 'ColorOrder', StandardColours,'NextPlot', 'replacechildren');
hold on
plot([0,0.25,0.5,0.75,1],[1,1,1,1,1].*X_Tstar(end,:)','+','lineWidth',1 )
xlabel('\chi_A')
ylabel('X_{T*}')
title('Raw Calculation')

subplot(3,2,6)
plot(ChiList(1:(end-1)),X_Tstar(1:(end-1),:)./max(X_Tstar(1:(end-1),:)),'lineWidth',1 )
set(gca, 'ColorOrder', StandardColours,'NextPlot', 'replacechildren');
hold on
plot([0,0.25,0.5,0.75,1],[1,1,1,1,1].*(X_Tstar(end,:)./max(X_Tstar(1:(end-1),:)))','+','lineWidth',1 )
xlabel('\chi_A')
ylabel('X_{T*}')
title('Rescaled')
ylim([-0.1,1.1])
