...
lockedParams=[0.02,1,0.1,0.02,0.04,0.04,0.17,0.75,0.01,0,0,0];
...
y0=[min(cyclePredict(:,1));cyclePredict(:,1);0.8;1-sum([min(cyclePredict(:,1));cyclePredict(:,1)]) ]
y0=y0./sum(y0);
y0(4)=0.57;
y0=y0./sum(y0);
y0(4)=0.57;
y0=y0./sum(y0);
y0(4)=0.57;
[t,yQQ]=ode45(ABRsim, [0,500], y0, odeset('NonNegative',1))
figure(43);
plot(t,yQQ);
hold on
%plot(t_cycle,cyclePredict,'*');
...
logR_ab = log(yQQ(:,4))
Teffect= (lockedParams(1)+ lockedParams(9))*(t-t(1))  % (mu+gam)*t.
Infectivity= lockedParams(3)*(1-lockedParams(7));

figure(5);
plot(t,logR_ab-logR_ab(1)+Teffect)
hold on
plot(t(2:end),Infectivity*cumsum(yQQ(2:end,5).*diff(t)) )
