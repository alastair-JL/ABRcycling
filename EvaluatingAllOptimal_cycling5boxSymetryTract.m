...
fineness=150;
Tlist= linspace(0,200,50);
rhoList=linspace(0,1,40);
%Tlist=[0,Tlist];
Result=0*Tlist;
...
MutantArrivalRates= zeros(length(rhoList),4);
M_IMPORT=1;
M_BASE=2;
M_SELECT=3;
M_HGT=4;
...
CycleVector=zeros(length(rhoList),5);

yFinal=[Sequil;Aequil;Bequil;Xequil;0];

extra=8;
ExitNumbers=-ones(length(rhoList),4+extra);
Maxs=-ones(length(rhoList),1);
Mins=-ones(length(rhoList),1);
Means=-ones(length(rhoList),1);
meanDownSwing=-ones(length(rhoList),1);
meanUpSwing=-ones(length(rhoList),1);
...
X365=ones(length(rhoList),4);
X365small=ones(length(rhoList),4);
Thalf=ones(length(rhoList),4);
X_T=ones(length(rhoList),4);
X_Tstar=ones(length(rhoList),4);
X_Tstar_Bastard=ones(length(rhoList),4);
X_Tstar_small=ones(length(rhoList),4);

XbarRecord=ones(length(rhoList),1);
MbarRecord=ones(length(rhoList),4);
expectedMutationTimesRecord=ones(length(rhoList),4);
expectedMutationTimesRecordSmall=ones(length(rhoList),4);




for(lll=1:(length(rhoList)))

T=80;

asymetryBias=rhoList(lll)*2-1;

yFinal=[rand(4,1);0]


chi=@(t) (mod(t,2*T)< T*(1+asymetryBias)).*1;
...
[tOut,yOut] = ode45(Deriv,linspace(T/2,warmupTime,40000),[yFinal; 10*ones(4,1)/nu(1); 10*ones(4,1)/nuSmall(1); zeros(17,1)]);
...
[tOutReference,yOutReference] = ode45(Deriv,linspace(0,2*T,fineness+1),[yOut(end,1:21)';zeros(25,1)]);
...
[tOut,yOut,tEvent,yEvent,~] = ode45(DerivWeave,[0,1365],[yStart'],Opt);
...
%These are pretty detailed NUMERICAL approximations.
            if(max(DelayTime)>9999999)    
                ...
                X365small(lll,Mtype)=Xcycle./(2*T)*365;
                    ...
                    Thalf(lll,Mtype)= expectedMutationTimes(Mtype) + integral(toIntegrate,0,2*T)/2*T;
            
                    toIntegrate= @(t) delayXFromTime(t).*PDF(t,Mtype);
                    X_T(lll,Mtype)= expectedMutationXintegral(Mtype) + integral(toIntegrate,0,2*T)/2*T;
...
StandardColours=[0, 0.4470, 0.7410;0.8500, 0.3250, 0.0980;0.9290, 0.6940, 0.1250;0.4940, 0.1840, 0.5560];
figure(1)
subplot(3,2,1)
set(gca, 'ColorOrder', StandardColours,'NextPlot', 'replacechildren');
hold on
plot(rhoList,X365,'--','lineWidth',2 )
xlabel('cycle bias')
ylabel('X_{365}')
...
set(gca, 'ColorOrder', StandardColours,'NextPlot', 'replacechildren');
hold on
plot(rhoList,X365small,'--','lineWidth',2)
xlabel('cycle bias')
ylabel('X_{365}')
...
set(gca, 'ColorOrder', StandardColours,'NextPlot', 'replacechildren');
hold on
plot(rhoList,Thalf,'--','lineWidth',2)
xlabel('cycle bias')
ylabel('T_{1/2}')
hold on
...
set(gca, 'ColorOrder', StandardColours,'NextPlot', 'replacechildren');
hold on
plot(rhoList,X_Tstar_Bastard,'--','lineWidth',2)
xlabel('cycle bias')
ylabel('X_{T*}')
title('Raw')
hold on
...
set(gca, 'ColorOrder', StandardColours,'NextPlot', 'replacechildren');
hold on
plot(rhoList,X_Tstar_Bastard./max(X_Tstar_Bastard),'--','lineWidth',2)
xlabel('cycle bias')
ylabel('X_{T*}')
